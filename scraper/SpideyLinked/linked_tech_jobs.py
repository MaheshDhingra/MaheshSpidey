...
class Linkedin():
    def getData(self):
        ...
        driver.find_element_by_id('username').send_keys(
            'USER NAME')  # Enter username of linkedin account here
        driver.find_element_by_id('password').send_keys(
            'PASSWORD')  # Enter Password of linkedin account here
        driver.find_element_by_xpath("//*[@type='submit']").click()

        search_key = "tech jobs"
        ...
        for no in range(1, 30):
            ...
            peoples = search.findAll(
                'a', attrs={'data-control-name': 'search_srp_result'})
            count = 0
            ...
            for people in peoples:
                ...
                if count % 2 == 0:
                    ...
                    try:
                        cover = page.find('img', attrs={
                                          'class': 'profile-background-image__image relative full-width full-height'})['src']
                    except:
                        cover = 'None'
                    ...
                    try:
                        title = str(page.find("li", attrs={
                                    'class': 'inline t-24 t-black t-normal break-words'}).text).strip()
                    except:
                        title = 'None'
                    try:
                        heading = str(
                            page.find('h2', attrs={'class': 'mt1 t-18 t-black t-normal'}).text).strip()
                    except:
                        heading = 'None'
                    try:
                        loc = str(page.find(
                            'li', attrs={'class': 't-16 t-black t-normal inline-block'}).text).strip()
                    except:
                        heading = 'None'
                    ...
                    details = info.findAll(
                        'section', attrs={'class': 'pv-contact-info__contact-type'})
                    try:
                    ...
                    data.append({'profile_url': profile_url, 'cover': cover, 'profile': profile, 'title': title, 'heading': heading,
                                'loc': loc, 'website': website, 'phone': phone, 'email': email, 'connected': connected, })
            print("!!!!!! Data scrapped !!!!!!")
